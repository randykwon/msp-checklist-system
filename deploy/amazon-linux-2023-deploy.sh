...
# MSP Checklist - Amazon Linux 2023 자동 배포 스크립트
# 이 스크립트는 Amazon Linux 2023에서 MSP Checklist 시스템을 완전히 설정합니다.
...
echo -e "${CYAN}"
echo "╔════════════════════════════════════════════════════════════╗"
echo "║         MSP Checklist - Amazon Linux 2023 배포            ║"
echo "║                                                            ║"
echo "║  이 스크립트는 Amazon Linux 2023에서 MSP Checklist        ║"
...
if [ -f /etc/os-release ]; then
    ...
    if [[ "$ID" != "amzn" ]] || [[ "$VERSION_ID" != "2023" ]]; then
        log_warning "이 스크립트는 Amazon Linux 2023용으로 최적화되었습니다."
...
# 기존 node_modules 정리 (문제 방지)
if [ -d "node_modules" ]; then
    rm -rf node_modules package-lock.json
fi

# npm 설정 최적화 (Amazon Linux 2023용)
...
# 14단계: 백업 및 모니터링 스크립트 설정
log_info "14단계: 백업 및 모니터링 설정 중..."
...
# 데이터베이스 백업
cp /opt/msp-checklist/msp-checklist/*.db "$BACKUP_DIR/db_backup_$DATE/"
cp /opt/msp-checklist/msp-checklist/admin/*.db "$BACKUP_DIR/db_backup_$DATE/" 2>/dev/null || true

# 로그 백업
tar -czf "$BACKUP_DIR/logs_backup_$DATE.tar.gz" /opt/msp-checklist/logs/
...
log_success "MSP Checklist 시스템이 Amazon Linux 2023에 성공적으로 배포되었습니다!"

echo ""
echo "서비스 접속 주소:"
PUBLIC_IP=$(curl -s http://checkip.amazonaws.com/ || echo "IP 확인 실패")
...
echo ""
log_success "Amazon Linux 2023 배포가 완전히 완료되었습니다! 🚀"
