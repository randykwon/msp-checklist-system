...
# Amazon Linux 2023에서 발생하는 LightningCSS 네이티브 모듈 오류 완전 해결
...
echo -e "${BLUE}🔧 즉시 LightningCSS 문제 해결${NC}"
echo "=================================="

PROJECT_DIR="./msp-checklist"
...
# 3. 문제가 있는 패키지들 package.json에서 완전 제거
log_info "package.json에서 문제 패키지들 완전 제거 중..."

# package.json 백업
cp package.json package.json.backup

# 문제가 있는 패키지들을 package.json에서 제거
cat package.json | \
jq 'del(.dependencies.lightningcss, 
       .dependencies."@tailwindcss/postcss", 
       .dependencies."@tailwindcss/node", 
       .dependencies.tailwindcss, 
       .dependencies.postcss, 
       .dependencies.autoprefixer,
       .devDependencies.lightningcss,
       .devDependencies."@tailwindcss/postcss",
       .devDependencies."@tailwindcss/node",
       .devDependencies.tailwindcss,
       .devDependencies.postcss,
       .devDependencies.autoprefixer)' > package.json.tmp && mv package.json.tmp package.json
...
# 5. globals.css를 완전히 새로 작성
log_info "globals.css 완전 재작성 중..."
cat > app/globals.css << 'EOF'
/* MSP Checklist 기본 CSS - LightningCSS 없이 */
...
# 6. Next.js 설정을 완전히 새로 작성 (LightningCSS 완전 제거)
...
cat > .env.local << 'EOF'
# MSP Checklist 환경 변수 (LightningCSS 없이)
NODE_ENV=production
PORT=3010
HOST=0.0.0.0
...
if [ -d "admin" ]; then
    ...
    # Admin package.json에서도 문제 패키지 제거
...
echo ""
log_success "🎉 LightningCSS 문제 완전 해결 완료!"
...
echo "  ✅ 간단한 CSS 프레임워크로 완전 교체"
echo "  ✅ Next.js 설정 완전 최적화"
