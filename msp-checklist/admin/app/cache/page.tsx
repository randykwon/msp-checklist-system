...
export default function CachePage() {
  ...
  const generateCache = async () => {
    ...
    try {
      ...
      const response = await fetch(`${mainAppUrl}/api/advice-cache-stream`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          options: { 
  ...
  const loadSummaryFiles = async () => {
    try {
      ...
      const response = await fetch('/api/cache-files?action=list&type=advice-summary');
      if (response.ok) {
        const data = await response.json();
  ...
  const handleImportSummaryFromFile = async () => {
    ...
    try {
      ...
      // 파일 읽기
      const readResponse = await fetch(`/api/cache-files?action=read&type=advice-summary&filename=${encodeURIComponent(selectedSummaryFile)}`);
      if (!readResponse.ok) {
      ...
      const importResponse = await fetch('/api/advice-summary', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'import',
  ...
  const loadCacheFilesFromDirectory = async () => {
    try {
      ...
      const response = await fetch('/api/cache-files?action=list&type=advice');
      if (response.ok) {
        const data = await response.json();
  ...
  const loadCacheFromFile = async (filename: string) => {
    try {
      ...
      // 파일 내용 읽기
      const readResponse = await fetch(`/api/cache-files?action=read&type=advice&filename=${encodeURIComponent(filename)}`);
      if (!readResponse.ok) {
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontWeight: 600, fontSize: 15 }}>
                    {progressInfo.phase === 'korean' ? '🇰🇷 한국어 조언 생성 중...' : 
                     progressInfo.phase === 'english' ? '🌐 영어 조언 생성 중...' : 
                     '⏳ 준비 중...'}
                  </span>
                </div>
                <div style={{ 
              ...
              <div style={{
                ...
                <div style={{ padding: 24 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
                    <button
                      ...
                      style={{
                        ...
                        color: importTab === 'file' ? 'white' : '#42B883',
                        background: importTab === 'file' ? 'linear-gradient(135deg, #42B883 0%, #35495E 100%)' : '#E8F5E9',
                        border: 'none', borderRadius: 10, cursor: 'pointer'
                      }}
                    >
                      📂 파일 업로드
                    ...
                    <button
                      ...
                      style={{
                        flex: 1, padding: '12px 16px', fontSize: 14, fontWeight: 600,
                        color: importTab === 'directory' ? 'white' : '#42B883',
                        background: importTab === 'directory' ? 'linear-gradient(135deg, #42B883 0%, #35495E 100%)' : '#E8F5E9',
                        border: 'none', borderRadius: 10, cursor: 'pointer'
                      }}
                    >
                      📁 캐시 폴더에서 로드
                  ...
                  {importTab === 'directory' && (
                    <div style={{ marginBottom: 20 }}>
                      <p style={{ margin: '0 0 16px', fontSize: 14, color: '#65676B', lineHeight: 1.6 }}>
                        <code style={{ background: '#F3F4F6', padding: '2px 6px', borderRadius: 4 }}>cache/advice/</code> 폴더에 저장된 캐시 파일을 선택하여 DB에 로드합니다.
                      </p>
                      {isLoadingFiles ? (
                          ...
                          {cacheFiles.map((file, index) => (
                            <div
                              ...
                              style={{
                                ...
                                background: selectedCacheFile === file.filename ? '#E8F5E9' : 'white',
                                cursor: 'pointer',
                      ...
                      {cacheFiles.length > 0 && (
                        <button
                          ...
                          style={{
                            ...
                            color: 'white',
                            background: !selectedCacheFile || isImporting ? '#D1D5DB' : 'linear-gradient(135deg, #42B883 0%, #35495E 100%)',
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {itemSummaryVersions.map((v, idx) => {
                      ...
                      const isActiveVersion = versionLang === 'ko' 
                        ? activeSummaryVersions.advice.ko === v.version
                        : activeSummaryVersions.advice.en === v.version;
                      ...
                      return (
                        <div 
                          ...
                          style={{
                            ...
                            background: isSelected ? '#ECFDF5' : isActiveVersion ? '#FEF3C7' : 'white',
                            border: `2px solid ${isSelected ? '#10B981' : isActiveVersion ? '#F59E0B' : '#E5E7EB'}`,
                            transition: 'all 0.2s'
                          }}
                        >
                          <div 
          ...
          >
            <div style={{ 
              ...
              <div>
                ...
                <p style={{ margin: '4px 0 0', fontSize: 12, opacity: 0.9 }}>
                  cache/advice-summaries/ 폴더의 파일을 선택하세요
                </p>
              </div>
              <button 
                onClick={() => setShowSummaryImportModal(false)}
            ...
            <div style={{ padding: 20, maxHeight: 'calc(80vh - 160px)', overflowY: 'auto' }}>
              {isLoadingSummaryFiles ? (
                ...
                <div style={{ textAlign: 'center', padding: 40, color: '#6B7280' }}>
                  <div style={{ fontSize: 48, marginBottom: 16 }}>📭</div>
                  <p>cache/advice-summaries/ 폴더에 파일이 없습니다.</p>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
