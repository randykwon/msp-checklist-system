...
export function initializeDatabase() {
  ...
  db.exec(`
    CREATE INDEX IF NOT EXISTS idx_agent_chat_user_id
    ON agent_chat_history(user_id, created_at)
  `);
}

// Agent chat functions
export function getAgentChatHistory(userId: number, limit: number = 20): { role: string; message: string; created_at: string }[] {
  return db.prepare(
    'SELECT role, message, created_at FROM agent_chat_history WHERE user_id = ? ORDER BY created_at DESC LIMIT ?'
  ).all(userId, limit) as { role: string; message: string; created_at: string }[];
}

export function saveAgentChatMessage(userId: number, role: 'user' | 'assistant', message: string): void {
  db.prepare('INSERT INTO agent_chat_history (user_id, role, message) VALUES (?, ?, ?)').run(userId, role, message);
}

export function clearAgentChatHistory(userId: number): void {
  db.prepare('DELETE FROM agent_chat_history WHERE user_id = ?').run(userId);
}
