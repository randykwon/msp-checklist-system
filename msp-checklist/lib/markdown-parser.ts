 ...
 * 다크 모드 지원: color: inherit 사용으로 부모 요소 색상 상속
 */
...
export function parseMarkdownToHtml(markdown: string): string {
  ...
  // 헤더 (## 또는 **헤더**) - inherit 사용
  html = html.replace(/^### (.+)$/gm, '<h4 style="font-size: 16px; font-weight: 700; color: inherit; margin: 20px 0 12px 0; padding-bottom: 8px; border-bottom: 2px solid currentColor; opacity: 0.3;">$1</h4>');
  html = html.replace(/^## (.+)$/gm, '<h3 style="font-size: 18px; font-weight: 700; color: inherit; margin: 24px 0 14px 0; padding-bottom: 10px; border-bottom: 2px solid currentColor; opacity: 0.3;">$1</h3>');
  html = html.replace(/^# (.+)$/gm, '<h2 style="font-size: 20px; font-weight: 700; color: inherit; margin: 28px 0 16px 0; padding-bottom: 12px; border-bottom: 3px solid currentColor; opacity: 0.3;">$1</h2>');
  ...
  // 수평선 (--- 또는 ***) - currentColor 사용
  html = html.replace(/^(---|\*\*\*)$/gm, '<hr style="border: none; border-top: 1px solid currentColor; margin: 16px 0; opacity: 0.3;" />');
