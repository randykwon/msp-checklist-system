...
// 현재 스크립트를 TypeScript로 실행
const tsScriptPath = path.join(__dirname, 'generate-advice-cache.ts');

try {
  execSync(`npx ts-node ${tsScriptPath} ${process.argv.slice(2).join(' ')}`, {
    stdio: 'inherit',
  ...
  try {
    switch (command) {
      ...
      case 'stats':
        ...
        const stats = cacheService.getCacheStats(version);
...
Usage: node scripts/generate-advice-cache.js <command> [options]
...
Examples:
  node scripts/generate-advice-cache.js generate
  node scripts/generate-advice-cache.js list
  node scripts/generate-advice-cache.js stats 20241217_143022
  node scripts/generate-advice-cache.js export 20241217_143022
  node scripts/generate-advice-cache.js import cache/advice_cache_20241217_143022.json
        `);
        break;
    }
  } catch (error) {
