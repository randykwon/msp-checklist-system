...
# MSP Checklist - Ubuntu 24.04 LTS (Noble Numbat) 자동 배포 스크립트
# AWS AMI: ubuntu-noble-24.04-amd64-server-20251022
# 이 스크립트는 Ubuntu 24.04 LTS에서 MSP Checklist 시스템을 완전히 설정합니다.
...
echo -e "${CYAN}"
echo "╔════════════════════════════════════════════════════════════╗"
echo "║     MSP Checklist - Ubuntu 24.04 LTS (Noble) 배포        ║"
echo "║                                                            ║"
echo "║  AWS AMI: ubuntu-noble-24.04-amd64-server-20251022        ║"
echo "║  이 스크립트는 Ubuntu 24.04 LTS에서 MSP Checklist        ║"
...
if [ -f /etc/os-release ]; then
    ...
    if [[ "$ID" != "ubuntu" ]]; then
        log_warning "이 스크립트는 Ubuntu 24.04 LTS용으로 최적화되었습니다."
...
read -p "GitHub 저장소 URL (기본값: $REPO_URL, 엔터로 기본값 사용): " INPUT_REPO
...
# 3단계: Node.js 20.x LTS 설치 (Ubuntu 24.04 호환)
log_info "3단계: Node.js 20.x LTS 설치 중..."
...
ENVEOF
log_success "메인 앱 환경 변수 파일 생성 완료"
...
# 9단계: 기본 관리자 계정 생성
log_info "9단계: 기본 관리자 계정 생성 중..."
cd "$APP_DIR"

# 관리자 계정 생성 스크립트 실행
if [ -f "create-admin.cjs" ]; then
    node create-admin.cjs || log_warning "관리자 계정이 이미 존재하거나 생성 실패"
fi

cd msp-checklist

# 10단계: PM2 설정
log_info "10단계: PM2 프로세스 관리 설정 중..."
...
# PM2 시작 스크립트 설정 (Ubuntu 24.04 systemd)
sudo env PATH=$PATH:/usr/bin pm2 startup systemd -u $USER --hp /home/$USER

log_success "PM2 설정 완료"

# 11단계: Nginx 설정
log_info "11단계: Nginx 설정 중..."
...
# 13단계: SSL 설정 (선택사항)
if [[ $SETUP_SSL =~ ^[Yy]$ ]]; then
    log_info "13단계: SSL 인증서 설정 중..."

    # Certbot 설치 (Ubuntu 24.04용 snap 사용)
    ...
    # 자동 갱신 설정 (snap certbot은 자동으로 타이머 설정됨)
    log_success "SSL 인증서 설정 완료"
fi

# 14단계: 시스템 서비스 등록
log_info "14단계: 시스템 서비스 등록 중..."
...
# 15단계: 백업 및 모니터링 스크립트 설정
log_info "15단계: 백업 및 모니터링 스크립트 설정 중..."
...
echo ""
echo "=== 네트워크 연결 ==="
ss -tlnp | grep -E ":301[01]" || netstat -tlnp | grep -E ":301[01]" || echo "포트 사용 정보 없음"
...
# cron 작업 설정
(crontab -l 2>/dev/null | grep -v "backup.sh" | grep -v "monitor.sh"; echo "0 2 * * * /opt/msp-checklist/backup.sh") | crontab -

log_success "백업 및 모니터링 설정 완료"

# 16단계: 최종 확인
log_info "16단계: 최종 시스템 확인 중..."
...
# 포트 확인
if ss -tlnp 2>/dev/null | grep -q ":3010" || netstat -tlnp 2>/dev/null | grep -q ":3010"; then
    log_success "메인 애플리케이션 포트 3010 활성화"
else
    log_warning "메인 애플리케이션 포트 3010 확인 필요"
fi

if ss -tlnp 2>/dev/null | grep -q ":3011" || netstat -tlnp 2>/dev/null | grep -q ":3011"; then
...
log_success "MSP Checklist 시스템이 Ubuntu 24.04 LTS에 성공적으로 배포되었습니다!"
...
if [ ! -z "$DOMAIN" ]; then
    if [[ $SETUP_SSL =~ ^[Yy]$ ]]; then
        ...
        echo "- 관리자 시스템: https://$DOMAIN:3011"
    else
        echo "- 메인 서비스: http://$DOMAIN"
        echo "- 관리자 시스템: http://$DOMAIN:3011"
    fi
else
...
echo ""
log_success "Ubuntu 24.04 LTS (Noble Numbat) 배포가 완전히 완료되었습니다! 🚀"
