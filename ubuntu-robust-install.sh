...
# MSP Checklist 강화된 설치 스크립트 (Ubuntu 22.04 LTS)
...
check_system_requirements() {
    ...
    if sudo lsof /var/lib/dpkg/lock-frontend 2>/dev/null; then
        log_error "다른 APT 프로세스가 실행 중입니다. 잠시 후 다시 시도하세요."
        exit 1
    fi
...
cleanup_existing_installation() {
    ...
    for port in 3010 3011; do
        PID=$(sudo netstat -tlnp 2>/dev/null | grep ":$port " | awk '{print $7}' | cut -d'/' -f1 | head -1)
        if [ ! -z "$PID" ]; then
...
update_system() {
    ...
    # APT 소스 업데이트
    retry_command "sudo apt update" "APT 소스 업데이트"
    
    # 시스템 업그레이드
    retry_command "sudo DEBIAN_FRONTEND=noninteractive apt upgrade -y" "시스템 업그레이드"
    
    # 필수 패키지 설치
    retry_command "sudo DEBIAN_FRONTEND=noninteractive apt install -y curl wget git build-essential software-properties-common python3 python3-pip" "필수 패키지 설치"
...
install_nodejs() {
    ...
    retry_command "curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash -" "NodeSource 저장소 추가"
    retry_command "sudo DEBIAN_FRONTEND=noninteractive apt-get install -y nodejs" "Node.js 설치"
